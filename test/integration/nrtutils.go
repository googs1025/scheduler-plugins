...
func waitForNRT(cs *clientset.Clientset) error {
	return wait.PollUntilContextTimeout(context.TODO(), 100*time.Millisecond, 3*time.Second,
...
func podIsScheduled(interval time.Duration, times int, cs clientset.Interface, podNamespace, podName string) (*corev1.Pod, error) {
	var err error
	var pod *corev1.Pod
	waitErr := wait.PollUntilContextTimeout(context.TODO(), interval, time.Duration(times)*interval,
